...
public class RefillOrderDB
{
    ...
    {
        Open();
        var OrderByProductIdList = connection.Query<RefillOrder>($@"SELECT r.refillorder_id,o.product_id,p.product_name,r.machine_id,r.order_date,o.product_price,sum(o.order_quantity) AS order_quantity, (product_price*sum(o.order_quantity))AS order_totalPay,r.employee_id,r.order_status,r.checkedBy_employee
        FROM (((products p
        LEFT JOIN orderdetails o ON p.product_id = o.product_id)
        LEFT JOIN refillorders r ON r.refillOrder_id = o.refillOrder_id)
        INNER JOIN employee e ON e.employee_id = r.employee_id)
        WHERE o.product_id = {number}
        GROUP BY r.refillOrder_id, r.machine_id;").ToList();
